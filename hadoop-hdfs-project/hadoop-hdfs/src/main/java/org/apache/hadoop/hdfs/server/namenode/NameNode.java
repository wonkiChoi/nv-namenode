...
public class NameNode implements NameNodeStatusMXBean {
  ...
  private static boolean format_nvram(Configuration conf, boolean force,
	      boolean isInteractive) throws IOException {
	    String nsId = DFSUtil.getNamenodeNameServiceId(conf);
	    String namenodeId = HAUtil.getNameNodeId(conf, nsId);
	    initializeGenericKeys(conf, nsId, namenodeId);
	    ...
	    // if clusterID is not provided - see if you can find the current one
	    String clusterId = StartupOption.NVRAM.getClusterId();
	    if(clusterId == null || clusterId.equals("")) {
	      //Generate a new cluster id
	      clusterId = NNStorage.newClusterID();
	    }
	    ...
	    try {
	      ...
	      fsImage.format_nvram(fsn, clusterId);
	    } catch (IOException ioe) {
	      LOG.warn("Encountered exception during format: ", ioe);
	      fsImage.close();
	      throw ioe;
  ...
  static StartupOption parseArguments(String args[]) {
    ...
    for(int i=0; i < argsLen; i++) {
      ...
      } else if (StartupOption.NVRAM.getName().equalsIgnoreCase(cmd)) {
          startOpt = StartupOption.NVRAM;
          for (i = i + 1; i < argsLen; i++) {
            if (args[i].equalsIgnoreCase(StartupOption.CLUSTERID.getName())) {
              i++;
              if (i >= argsLen) {
