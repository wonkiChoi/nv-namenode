...
//		LOG.info("[checking Time ] getTime = " + fsd.getTime);
	...
	static INodesInPath resolve(final INodeinNVRAM startingDir, final byte[][] components, final boolean resolveLink)
			throws UnresolvedLinkException {
//		long startresolve = System.currentTimeMillis();
		Preconditions.checkArgument(startingDir.compareTo(components[0]) == 0);
		INode curNode = startingDir;
		...
		while (count < components.length && curNode != null) {
			...
			final boolean isRef = curNode.isReference();
			final boolean isDir = curNode.isDirectory();
			final INodeinNVRAM dir = isDir ? (INodeinNVRAM)curNode : null;
			if (!isRef && isDir && dir.asDirectory().isWithSnapshot()) {
				// if the path is a non-snapshot path, update the latest
				// snapshot.
				if (!isSnapshot && shouldUpdateLatestId(dir.asDirectory().getDirectoryWithSnapshotFeature().getLastSnapshotId(),
						snapshotId)) {
					snapshotId = dir.asDirectory().getDirectoryWithSnapshotFeature().getLastSnapshotId();
				}
			} else if (isRef && isDir && !lastComp) {
				// If the curNode is a reference node, need to check its
				// dstSnapshot:
			...
			// check if the next byte[] in components is for ".snapshot"
			if (isDotSnapshotDir(childName) && dir.asDirectory().isSnapshottable()) {
				// skip the ".snapshot" in components
				count++;
				isSnapshot = true;
				// check if ".snapshot" is the last element of components
				...
				// Resolve snapshot root
				final Snapshot s = dir.asDirectory().getSnapshot(components[count + 1]);
				if (s == null) {
					curNode = null; // snapshot not found
				} else {
					curNode = s.getRoot();
